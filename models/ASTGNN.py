...
class Model(nn.Module):
    def __init__(self, DEVICE, encoder_input_size, decoder_output_size, d_model, adj_mx, nb_head, num_of_weeks,
               num_of_days, num_of_hours, points_per_hour, num_for_predict, num_layers=3, dropout=.0, aware_temporal_context=True,
        ...
        if ScaledSAt:  # employ spatial self attention
            ...
            position_wise_gcn = PositionWiseGCNFeedForward(spatialGCN(norm_Adj_matrix, d_model, d_model),
                                                           dropout=dropout)
        ...
        if aware_temporal_context:  # employ temporal trend-aware attention
            ...
            attn_st = MultiHeadAttentionAwareTemporalContex_qc_k1d(nb_head, d_model, num_of_weeks, num_of_days,
                                                                   num_of_hours, num_for_predict, kernel_size,
                                                                   dropout=dropout)
            att_tt = MultiHeadAttentionAwareTemporalContex_qc_kc(nb_head, d_model, num_of_weeks, num_of_days,
                                                                 num_of_hours, num_for_predict, kernel_size,
