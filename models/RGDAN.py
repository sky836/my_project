...
class STEmbModel(torch.nn.Module):
    ...
    def forward(self, SE, TE, T=288):
        ...
        '''dayofweek = F.one_hot(TE[..., 1], num_classes = 7)
        timeofday = F.one_hot(TE[..., 0], num_classes = self.TEDims-7)
...
class Model(torch.nn.Module):
    def __init__(self, configs, adj, K=8, d=8, L=1):
        ...
        # spatial embedding
        with open(r'datasets/PEMS03/pems03se.txt', mode='r') as f:
            lines = f.readlines()
            temp = lines[0].split(' ')
        ...
        SEDims = self.SE.shape[1]
        TEDims = configs.steps_per_day + 7
        ...
        self.fc27 = torch.nn.Linear(D, configs.output_dim)
        self.nodevec1 = nn.Parameter(torch.randn(num_nodes, 10).to(device), requires_grad=True).to(device)
        self.nodevec2 = nn.Parameter(torch.randn(10, num_nodes).to(device), requires_grad=True).to(device)
