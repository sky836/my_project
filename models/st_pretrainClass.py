...
class Model(nn.Module):
    def __init__(self, configs, supports=None):
        ...
        # ===================================pretrain special=============================================
        self.token_T = nn.init.xavier_uniform_(nn.Parameter(torch.zeros(1, 1, self.time_dim)))
        self.token_S = nn.init.xavier_uniform_(nn.Parameter(torch.zeros(1, 1, 1, self.target_dim)))
        self.time_proj = nn.Linear(self.time_dim, self.feed_forward_dim)
        self.target_proj = nn.Linear(self.target_dim*self.num_nodes, self.feed_forward_dim)
        self.class_end1 = nn.Linear(self.feed_forward_dim*2, self.feed_forward_dim)
        self.class_end2 = nn.Linear(self.feed_forward_dim, 2)
    ...
    def encoding(self, x):
        ...
        # cat class token
        token_T = self.token_T.expand(batch_size, 1, self.time_dim)
        token_S = self.token_S.expand(batch_size, 1, num_nodes, self.target_dim)

        time_features = torch.cat([token_T, time_features], dim=1)
        target_features = torch.cat([token_S, target_features], dim=1)
    ...
    def forward(self, x):
        ...
        class_token_T = time_features[:, 0]  # [b, time_dim]
        class_token_S = target_features[:, 0].reshape(batch_size, -1) # [b, num_nodes*target_dim]

        class_token_T = self.time_proj(class_token_T)
        class_token_S = self.target_proj(class_token_S)

        class_token = torch.cat([class_token_T, class_token_S], dim=-1)  # [b, feed_forward_dim*2]
        class_token = self.class_end1(class_token)
        class_token = F.relu(class_token)
        out = self.class_end2(class_token)
